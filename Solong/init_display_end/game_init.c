...
/*   Updated: 2021/08/09 14:01:23 by fmonbeig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	img_init(t_game *g)
{
	...
	if (!g->exit)
		error_malloc(g);
	g->player->img = mlx_xpm_file_to_image(g->mlx, "./images/player.xpm",
			&g->player->width, &g->player->height);
	g->collec->img = mlx_xpm_file_to_image(g->mlx, "./images/collec.xpm",
			&g->collec->width, &g->collec->height);
	g->wall->img = mlx_xpm_file_to_image(g->mlx, "./images/wall.xpm",
			&g->wall->width, &g->wall->height);
	g->floor->img = mlx_xpm_file_to_image(g->mlx, "./images/floor_lol.xpm",
			&g->floor->width, &g->floor->height);
	g->exit->img = mlx_xpm_file_to_image(g->mlx, "./images/exit.xpm",
